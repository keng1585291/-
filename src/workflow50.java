...
public class workflow50{
	private static List<Cloudlet> cloudletList;  //tasks lists
	
	private static int cloudletNum=50;	//total number of tasks

	private static List<Vm> vmList;	//Virtual machine lists
	private static int vmNum=5; //total number of vms
	...
	public static void main(String[] args){
		...
		try{
			...
			int vmid=0;
			int[] mipss=new int[]{456,510,519,532,632};//,241,424,235,365,578,856,436,746,436,235};
			long size=10000;
			int ram=2048;
			long bw=1000;
			int pesNumber=1;
			...
			storageList.get(26).addFile(file);
			cloudletList.get(26).addRequiredFile(Integer.toString(cloudletList.get(i).getCloudletId()));
			}
			for(int i=13;i<26;i++)
			{File file=new File
			(Integer.toString(cloudletList.get(i).getCloudletId()),(int)cloudletList.get(i).getCloudletOutputSize());
			storageList.get(27).addFile(file);
			cloudletList.get(27).addRequiredFile(Integer.toString(cloudletList.get(i).getCloudletId()));
			}
			for(int i=28;i<40;i++){
			File file=new File
			(Integer.toString(cloudletList.get(26).getCloudletId()),(int)cloudletList.get(26).getCloudletOutputSize());
			storageList.get(i).addFile(file);
			cloudletList.get(i).addRequiredFile(Integer.toString(cloudletList.get(26).getCloudletId()));

			}
			for(int i=40;i<49;i++)
			{
				File file=new File
				(Integer.toString(cloudletList.get(27).getCloudletId()),(int)cloudletList.get(27).getCloudletOutputSize());
				storageList.get(i).addFile(file);
				cloudletList.get(i).addRequiredFile(Integer.toString(cloudletList.get(27).getCloudletId()));
			}
			...
			File file=new File
			(Integer.toString(cloudletList.get(i).getCloudletId()),(int)cloudletList.get(i).getCloudletOutputSize());
			storageList.get(49).addFile(file);
			cloudletList.get(49).addRequiredFile(Integer.toString(cloudletList.get(i).getCloudletId()));
			}
